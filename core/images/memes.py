...
def megamind(text):
    ...
    wrapped = textwrap.wrap(
        text, 20, break_long_words=True, max_lines=2, placeholder="")
...
def fate(string, scissors, other):
    ...
    string_lines = textwrap.wrap(
        string, 20, break_long_words=True, break_on_hyphens=True, max_lines=3, placeholder="")

    for (index, line) in enumerate(string_lines):
        line_size = draw.textsize(line, font=font)
        draw.text(
            ((X_MARGIN-line_size[0])/2+X_MARGIN,
             (Y_MARGIN-line_size[1])/2+Y_MARGIN+40*index),
            line,
            font=font
        )
    ...
    scissor_lines = textwrap.wrap(
        scissors, 20, break_long_words=True, break_on_hyphens=True, max_lines=3, placeholder="")
    ...
    other_lines = textwrap.wrap(
        other, 20, break_long_words=True, break_on_hyphens=True, max_lines=3, placeholder="")
...
def wtfdidyoujustdo(image_url, AMOUNT, DURATION):
    ...
    MARGIN_X, MARGIN_Y = round(
        (canvas.width-question.width)/2), round((canvas.height-question.height)/2)
    ...
    img = BytesIO()
    canvas.save(img, format="GIF", save_all=True,
                append_images=images, loop=0, duration=DURATION)
    img.seek(0)
...
def obama(receiver, giver):
    ...
    receiver_width, receiver_height = draw.textsize(
        receiver, font, stroke_width=2)

    draw.text(
        (420 + (canvas_width-receiver_width)/2,
         400 + (canvas_height-receiver_height)/2),
        receiver,
        font=font,
        stroke_fill="black",
        stroke_width=2
...
def monke(ein, zwei, drei_name, drei_text):
    ...
    lines = textwrap.wrap(ein, 10, max_lines=3, placeholder="",
    ...
    for (index, line) in enumerate(lines):
        ...
        draw.text(
            (100 + (canvas_width-text_width)/2, 250 +
             (canvas_height-text_height)/2 + (font.size+5)*index),
            line,
            font=font,
            stroke_fill="black",
            stroke_width=2
    ...
    lines = textwrap.wrap(zwei, 10, max_lines=3, placeholder="",
                          break_long_words=True, break_on_hyphens=True)
    ...
    for (index, line) in enumerate(lines):
        ...
        draw.text(
            (380 + (canvas_width-text_width)/2, 220 +
             (canvas_height-text_height)/2 + (font.size+5)*index),
            line,
            font=font,
            stroke_fill="black",
    ...
    lines = textwrap.wrap(drei_name, 20, max_lines=2, placeholder="",
                          break_long_words=True, break_on_hyphens=True)
    ...
    for (index, line) in enumerate(lines):
        ...
        draw.text(
            (760 + (canvas_width-text_width)/2, 30 +
             (canvas_height-text_height)/2 + (font.size+5)*index),
            line,
            font=font,
            stroke_fill="black",
    ...
    lines = textwrap.wrap(drei_text, 20, max_lines=3, placeholder="",
                          break_long_words=True, break_on_hyphens=True)
    ...
    for (index, line) in enumerate(lines):
        ...
        draw.text(
            (760 + (canvas_width-text_width)/2, 220 +
             (canvas_height-text_height)/2 + (font.size+5)*index),
            line,
            font=font,
            stroke_fill="black",
...
def stats(stat: str):
    ...
    lines = textwrap.wrap(stat, 10, break_long_words=True,
                          break_on_hyphens=True, placeholder="", max_lines=3)
    ...
    for (index, line) in enumerate(lines):
        ...
        draw.text(
            (120 + (canvas_width-text_width)/2, canvas.height-165-20 *
             (len(lines)-1) + (canvas_height-text_height)/2 + (font.size+2)*index),
            line,
            fill="black",
            font=font
        )
...
def what(image, main: str, sub=""):
    ...
    image = ImageOps.fit(
        image, (min(canvas.width-40, image.width), SIZE), centering=(.5, .5))
    ...
    draw.text((round((canvas.width-text_width)/2),
              image.height+70), main, "white", font=font)
    ...
    draw.text((round((canvas.width-text_width)/2),
              image.height+130), sub, "white", font=font)
...
def we_are_not_the_same(top_text, bottom_text):
    ...
    text_width, _ = draw.multiline_textsize(
        f"{top_text}\n\n\n{bottom_text}", font=font)

    draw.multiline_text(((canvas.width-text_width)/2, 600),
                        f"{top_text}\n\n\n{bottom_text}", font=font, align="center")

    text_width, text_height = draw.textsize("We are not the same", font=font)
    draw.text(((canvas.width-text_width)/2, canvas.height -
              text_height-40), "We are not the same", font=font)
...
def stuff_admini(text: str):
    ...
    draw.multiline_text((270+(canvas.width-text_width)/2, 100),
                        text, font=font, align="center", fill="black")
...
def thing_at_home(image: BytesIO, thing: str = "BTB"):
    ...
    IMAGE_HEIGHT_MARGIN = font.size + \
        (font.size*(len(mom_text)+len(me_text)+len(at_home_text)))+20
    ...
    image: Image.Image = Image.open(image).resize(
        (canvas.width, canvas.height-IMAGE_HEIGHT_MARGIN))
...
def mass(text: str):
    ...
    draw.text((LEFT_MARGIN+round((canvas.width-LEFT_MARGIN-text_width)/2),
              15), text, "black", font=font, stroke_fill="black")
...
def jason(topic: str, description: str):
    ...
    description = textwrap.wrap(
        description, 30, max_lines=2, placeholder="    ")
    ...
    text_draw.multiline_text((220, 750, 0), '\n'.join(
        topic), "black", font=font, align='center')
...
def wizard_boom(text: str):
    canvas = Image.open("assets/MemeTemplates/WizardBoom.png")

    draw = ImageDraw.Draw(canvas)
    font = ImageFont.truetype("assets/fonts/impact.ttf", size=100)

    text_wrapped = '\n'.join(textwrap.wrap(text.upper(), 20, max_lines=3))
    text_width = draw.multiline_textsize(text_wrapped, font=font)[0]

    if text_width > canvas.width:
        font = ImageFont.truetype(
            "assets/fonts/impact.ttf",
            size=font.size - round(font.size*((text_width-canvas.width)/canvas.width))
        )

    text_width = draw.multiline_textsize(text_wrapped, font=font)[0]

    draw.multiline_text(
        (round((canvas.width-text_width) / 2), canvas.height - 120 - 50 * text_wrapped.count("\n")),
        text_wrapped,
        "white",
        font=font,
        align='center',
    ...
    return disnake.File(img, "wizzard_boom.png")

def wizard_think(text: str):
    canvas = Image.open("assets/MemeTemplates/WizardThink.png")
    ...
    return disnake.File(img, "wizard_think.png")
